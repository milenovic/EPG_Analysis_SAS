...
*********************************************************************
*********************************************************************;
%include 'C:\Users\milan\Desktop\IITA Work\EPG Data\SAScode\mult.sas'; run;
options ls=100 ps=72;
%let InPath = C:\Users\milan\Desktop\IITA Work\EPG Data\; *Folder with input file. WITH \ at the end please;
%let InFile = Platinum-Gold-Cassava-CST; *Input file name, Without extension please (it is assumed to be .csv);
%let OutPath = C:\Users\milan\Desktop\IITA Work\EPG Data\paperout-PvsG\; *Folder to put the results, WITH \ at the end please;
x "cd ""&OutPath.""";
...
*Finished finding min max mean median; 
*************************************;
*invert if median>mean so BoxCox does not complain. Why???;
 ...
 *Observations = (Observations-min)/(max-min); * rescale to 0-1, caused wrong lambda selection???;
Run;

**********************************************************
*** Find min max mean again. this is needed if other   ***
*** transformations should be applied before BoxCox    ***
*** and the use of min and max is needed for rescaling ***
**********************************************************;
ods exclude all;
ods graphics on;
ods output summary=ParamMeans; *NOTE: is there better way to get median in a table like we normally get means? ods out= did not work;
Proc means data=ebertlong min max; by Parameter;
Run;
Data ParamMeans (keep=Parameter observations_Max observations_Min rename=(observations_Max=max observations_Min=min));
set ParamMeans; 
run;
proc sort data=EbertLong (drop= min max mean median); by Parameter;
proc sort data=ParamMeans; by Parameter;
run;
Data EbertLong;
 MERGE EbertLong(IN=In1) ParamMeans;
...
**************************************************
*** Apply other transformation Before BoxCox.  ***
*** Useful if S-curve approximation is needed  ***
*** to achieve normality. E.g: Arcsine, Logit. ***
**************************************************;

Data EbertLong; set EbertLong;
If Observations ne "." then do;
 if parameter = "PotE2Indx" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "PrcntPrbE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TotDurNnPhlPhs" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE1FllwdE2PlsE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "maxE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "MnDurE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "PrcntPrbC" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "PrcntPrbE1" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TmLstE2EndRcrd" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TmLstPdEndRcrd" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TmStrtEPGFrstE" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TotDurNnPhlPhs" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurC" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE1" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE1FlldE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE1FlldSusE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE1FllwdE2PlsE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurE2" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else if parameter = "TtlDurSnglE1" then Observations = arsin(sqrt((observations-min)/(max-min)));
 else Observations = Observations;
end;
run;

*Trt is the independent variable, it needs to be numeric!;
Data EbertLong; set EbertLong;
trtnum=indexc(tranwrd('ABCDEFGHIJKLMNOPQRSTUVWXYZ',trim(left(upcase(trt))),'*'),'*');
run;
 
*Sort the data for BoxCox;
...
proc transreg detail nozeroconstant data=EbertLong nomiss; by Parameter; id insectno trt min max ;
model boxcox(observations/ LAMBDA= -5 TO 3 BY 0.05 PARAMETER=1)=identity(trtnum); output out=BoxCoxTransLong;
...
/****************************************************************************
*** Custom trans can also be defined here if one wants to avoid           ***
*** BoxCox power transform after custom transformation. Do it as follows: ***
*** Data BoxCoxTransLong; Set BoxCoxTransLong;                            ***
*** if parameter = "PrcntPrbE2" then Tobservations = trans(observations); ***
*** Note that if values need to be rescaled to 0-1 min and max need to be ***
*** calculated again. Note: I never found this beneficial.                ***
****************************************************************************/;

*Sort as required for PROC GLIMMIX BY statement, cleanup missing and not needed along the way;
...
*make q-q plots for every parameter;
ods exclude none;
ods graphics on;
proc univariate data=BoxCoxTransLong; by Parameter;
variables Transformed; qqplot;
run;
...
ods output LSMeans=LSMeansFull;
Proc glimmix data=BoxCoxTransLong plots=residualpanel;
by Parameter;
class trt;
model transformed=trt;
random _residual_/group=trt;
lsmeans trt/pdiff lines adjust=tukey alpha=0.05;
title "ANOVA and Tukey test";
run;
ods output close;


PROC SORT DATA=ANOVA(keep=Parameter) OUT=parameters 
 NODUPKEY ;
 BY Parameter ;
RUN ;
Data GroupsM; Set _null_; run; *prevent "Dataset does not exist" warning from the macro;
Data _null_; Set parameters;
*use Call Execute here;
Call execute ('
Data LSMeans; Set LSMeansFull;
where parameter="' || Parameter ||'";
run;
Data Diffs; Set ANOVA;
where parameter="' || Parameter || '";
run;
%nrstr(%mult(trt=trt));
run;');
run;
Data GroupsM; Set GroupsM;
Where lsmean ne .;
Letter=cats(of COL:);
keep parameter label Letter lsmean;
rename label=trt;
run;

*Cleanup;
proc delete lib=work data= Diffs LSMeans Letters Letters0-Letters3 Diffs0 LSMeans0 BoxCoxTransLong; run;
*LSMeansFull not deleted;
...
Data LSMeansFull; Set LSMeansFull;
Keep Parameter trt Lower Upper;
run;

*Finally merge the two datasets into a pretty table;
Proc sort data=work.trtMeansLong; by Parameter trt;
Proc sort data=work.GroupsM; by Parameter trt;
Proc sort data=work.LSMeansFull; by Parameter trt;
Data Final; Set trtMeansLong GroupsM LSMeansFull; 
merge trtMeansLong GroupsM LSMeansFull; by Parameter trt;
run;
...
proc delete lib=work data= trtMeansLong GroupsM lambda;
